...
<div class="row mb-4">
    @if (in_array('billers_create', $routes_user))
        <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-center mb-3">
            <div class="input-group">
                <a href="{{ route('billers_create') }}">
                    <button class="btn btn-warning ripple m-1 add-new-btn" type="button" @php if(isset($username) &&
                        $username !=='selada_produktif' ) echo 'disabled' @endphp>Add New</button>
                </a>
            </div>
        </div>
    @endif
                        ...
                        <tbody>
                            ...
                            @foreach($data as $item)
                                                        <tr>
                                                            ...
                                                            <td>{{ $item->email }}</td>
                                                            <td>
                                                                @if (in_array('billers_edit', $routes_user))
                                                                    <a href="{{ route('billers_edit', $item->id) }}">
                                                                        <button class="btn btn-edit ripple btn-sm m-1 edit-btn"
                                                                            type="button">Edit</button>
                                                                    </a>
                                                                @endif
                                                                @if (in_array('billers_destroy', $routes_user))
                                                                    <a href="#" onclick="deleteConfirm({{ $item->id }}); return false;"
...
<script>
    function deleteConfirm(id) {
        ...
        if (r == true) {
            var url = '{{ route("billers_destroy", [":id"]) }}';
