...
class TransactionBJBExport implements FromView, WithColumnFormatting, WithEvents, WithStyles, WithTitle, ShouldAutoSize
{
    use Exportable;
...
//	}
        ...
        if($this->request->has('fee')){
            if($this->request->get('fee') == 'true'){
                $data->where('fee', '<>', '0');
            }
        }
...
//	dd($data->toSql());die();
	...
	//return $data->query();
//	return $data;
	return view('exports.transactions_bjb', [
            'data' => $data
        ]);
   }
}
